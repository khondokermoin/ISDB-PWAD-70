...
try {
  ...
  $pdo = db();
  $stmt = $pdo->prepare("SELECT id, role, password_hash, is_active, employee_id, allowed_ip, allowed_mac FROM users WHERE username = ? OR email = ? OR phone = ? LIMIT 1");
...
if (!$u || !(int)$u['is_active'] || !password_verify($password, $u['password_hash'])) {
  ...
  $_SESSION['user'] = [
    ...
    'allowed_mac' => $u['allowed_mac'] ?? null,
    'employee' => $employee,
  ];
