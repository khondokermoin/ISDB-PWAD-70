...
// -----------------------------
// Prevents the background from scrolling while a modal is open.
// Also avoids layout shift by compensating for the scrollbar width.
let scrollLockY = 0;
...
setTimeout(() => {
  modalClose?.focus();
    ...
    document.addEventListener("click", (e) => {
      ...
      const btn = t.closest("[data-open-project]");
...
/* --------------------------
   Added cinematic interactions:
   Lenis + GSAP ScrollTrigger, Preloader, Three.js particles,
   Horizontal scroll, Magnetic buttons, Custom cursor, Devtools shortcut block.
---------------------------*/
(() => {
  const w = window;

  // Ensure libs exist (fail gracefully if missing)
  const gsap = w.gsap;
  ...
  // NOTE: If Lenis is initialized but its RAF loop doesn't run, mouse-wheel can feel "dead".
  // This setup runs Lenis via requestAnimationFrame (independent of GSAP), and syncs ScrollTrigger when available.
  let lenis;
  if (Lenis) {
    lenis = new Lenis({
      smoothWheel: true,
    ...
    // Always keep Lenis animating (so wheel works)
    const raf = (time) => {
      lenis.raf(time);
  ...
  w.addEventListener('load', () => {
    ...
    // Fallback: if GSAP doesn't load, don't leave the site stuck behind the preloader.
    if (!gsap) {
      if (preloader) preloader.style.display = 'none';
      // If the custom cursor animation won't run, restore the normal cursor.
    ...
    if (loadText) gsap.to(loadText, { opacity: 0, duration: 0.5 });
    if (preloader) {
      gsap.to(preloader, {
        y: '-100%',
        duration: 1.2,
        ...
        delay: 0.5,
        onComplete: () => {
          // Remove from the stacking context so it never blocks clicks.
  ...
  // (runs only if THREE is available)
  let renderer, camera, particles;
  if (THREE) {
    ...
    if (container) {
      ...
      particles = new THREE.Points(
        geometry,
        new THREE.PointsMaterial({ color: 0x444444, size: 2 })
      );
      scene.add(particles);
  ...
  // Resize handler (Three.js + ScrollTrigger)
  ...
  // 4) Horizontal Scroll logic (only if elements exist)
  if (gsap && ScrollTrigger) {
    ...
    if (horizontalContent && horizontalWrap) {
      gsap.to('#horizontal-content', {
        x: () => -(horizontalContent.scrollWidth - w.innerWidth),
        ease: 'none',
        scrollTrigger: {
  ...
  if (gsap && cursorOuter && cursorInner) {
    ...
    // 7) Cursor scale on interactive elements
  ...
  // 8) Security - Block Inspect (modernized)
  document.addEventListener('keydown', (e) => {
