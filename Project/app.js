...
(() => {
  const initModernPortfolioFx = () => {
    ...
    const initPreloader = () => {
      ...
      const heroText = document.querySelector('#about .space-y-6');
      const heroMedia = document.querySelector('#about .relative.flex.justify-center');
      if (!preloader) return;
      ...
      const revealTargets = [nav, heroText, heroMedia].filter(Boolean);
      const tl = gsap.timeline({ defaults: { ease: 'power3.out' } });

      tl.fromTo(text, { opacity: 0, scale: 0.92, y: 18 }, { opacity: 1, scale: 1, y: 0, duration: 0.8 })
        .to(text, { opacity: 0, y: -20, duration: 0.45, delay: 0.25 })
        .to(preloader, { yPercent: -100, duration: 1.05, ease: 'power4.inOut', onComplete: finish }, '-=0.1');

      if (revealTargets.length) {
        gsap.fromTo(revealTargets, { opacity: 0, y: 28 }, {
          opacity: 1,
          y: 0,
          duration: 1,
          stagger: 0.12,
      ...
      gsap.fromTo(preloader, { backgroundPosition: '0% 0%' }, { backgroundPosition: '100% 0%', duration: 1.4, ease: 'none' }, 0);
      if (text) {
        gsap.to(text, { textShadow: '0 0 34px rgba(34, 211, 238, 0.55)', repeat: 1, yoyo: true, duration: 0.4 }, 0.1);
      }
    };
    ...
    const initCustomCursor = () => {
      ...
      if (!ring || !dot || !hasFinePointer || !canAnimate) return;

      const state = { visible: false };
      const ringX = window.gsap ? gsap.quickTo(ring, 'x', { duration: 0.38, ease: 'power3.out' }) : (v) => { ring.style.left = `${v}px`; };
      const ringY = window.gsap ? gsap.quickTo(ring, 'y', { duration: 0.38, ease: 'power3.out' }) : (v) => { ring.style.top = `${v}px`; };
      const dotX = window.gsap ? gsap.quickTo(dot, 'x', { duration: 0.12, ease: 'power2.out' }) : (v) => { dot.style.left = `${v}px`; };
      const dotY = window.gsap ? gsap.quickTo(dot, 'y', { duration: 0.12, ease: 'power2.out' }) : (v) => { dot.style.top = `${v}px`; };

      const showCursor = () => {
        if (state.visible) return;
        state.visible = true;
        if (window.gsap) {
          gsap.to([ring, dot], { opacity: 1, duration: 0.18, overwrite: true });
        } else {
      ...
      document.addEventListener('mouseleave', () => {
        if (window.gsap) gsap.to([ring, dot], { opacity: 0, duration: 0.2, overwrite: true });
      });
