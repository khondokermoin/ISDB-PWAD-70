...
function handle_employees_update(int $id): void {
  ...
  $allowedUserFields = ['allowed_ip', 'allowed_device_tokens'];
  ...
  // Handle allowed_ip / allowed_device_tokens updates (users table)
  $userSet = [];
  $userVals = [];
  ...
  if (array_key_exists('allowed_device_tokens', $data)) {
    ...
    if ($allowed_device_tokens === '') {
      ...
      $userSet[] = "allowed_device_tokens = ?";
      $userVals[] = $allowed_device_tokens;
    }
  }
  ...
  $st2 = $pdo->prepare("SELECT e.*, u.allowed_ip, u.allowed_device_tokens
                         FROM employees e
