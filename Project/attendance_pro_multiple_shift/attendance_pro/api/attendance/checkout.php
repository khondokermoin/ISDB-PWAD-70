...
function handle_checkout(): void {
  ...
  // --- IP/Device token restriction ---
  $ip = client_ip();

  $body = get_json_body();
  $providedToken = trim((string)($_SERVER['HTTP_X_DEVICE_TOKEN'] ?? ($body['device_token'] ?? '')));

  $uStmt = $pdo->prepare('SELECT allowed_ip, allowed_device_tokens FROM users WHERE id = ? LIMIT 1');
  $uStmt->execute([(int)$user['id']]);
  $uRow = $uStmt->fetch();
  $userAllowed = trim((string)($uRow['allowed_ip'] ?? ''));
  $userAllowedDeviceTokens = trim((string)($uRow['allowed_device_tokens'] ?? ''));

  if ($userAllowed !== '' && !ip_allowed($ip, $userAllowed)) {
    json_response(['error' => 'You can only check-out from your assigned network'], 403);
  }

  if ($userAllowedDeviceTokens !== '' && !device_token_allowed($providedToken, $userAllowedDeviceTokens)) {
  ...
  $upd = $pdo->prepare('UPDATE attendance_logs SET check_out=?, total_minutes=?, overtime_minutes=?, early_leave_minutes=?, status=?, updated_at=? WHERE id=?');
  $upd->execute([$nowStr, $totalMinutes, $overtimeMinutes, $earlyLeaveMinutes, $status, now_dt(), (int)$log['id']]);

  audit_log((int)$user['id'], 'CHECKOUT', ['employee_id' => $employee_id, 'work_date' => $workDate, 'total_minutes' => $totalMinutes]);
  json_response(['ok' => true, 'work_date' => $workDate, 'check_out' => $nowStr, 'total_minutes' => $totalMinutes, 'overtime_minutes' => $overtimeMinutes, 'early_leave_minutes' => $earlyLeaveMinutes, 'status' => $status]);
}